...
public abstract class BaiduBlockChain implements InitializingBean {
    ...
    public Transaction invokeContractWithFee(Account account, String method, Map<String, String> params) {
        Transaction transaction = client.queryEVMContract(account, CONTRACT_NAME, method, params);
        log.info("transaction is {}", transaction.getContractResponse().getBodyStr());
        return client.invokeEVMContract(account, CONTRACT_NAME, method, params, BigInteger.TEN);
    }
